<template>
  <div class="flex flex-wrap items-center gap-1 p-1 sm:p-1.5 rounded-2xl bg-forest-950/90 backdrop-blur-md border border-forest-800 shadow-premium text-xs font-mono text-white max-w-full">
    <!-- Camera Reset Button -->
    <button
      type="button"
      @click="$emit('resetCamera')"
      class="flex items-center gap-1 px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl text-emerald-200 hover:text-white hover:bg-white/10 transition cursor-pointer text-[10px] sm:text-[11px]"
      title="Reset Sudut Pandang Kamera"
    >
      <Compass :size="13" class="text-gold-400 shrink-0" />
    ...
    <button
      ...
      class="flex items-center gap-1 px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl transition cursor-pointer text-[10px] sm:text-[11px]"
    ...
    >
      <RotateCw :size="13" :class="isAutoRotate ? 'animate-spin' : 'text-gold-400 shrink-0'" />
      <span class="font-bold">Orbit</span>
    </button>

    <div class="h-3 w-px bg-forest-700 mx-0.5" />
    ...
    <div class="flex items-center gap-0.5 sm:gap-1">
      <button
        ...
        class="flex items-center gap-1 px-1.5 sm:px-2.5 py-1 sm:py-1.5 rounded-lg text-[9px] sm:text-[11px] font-bold transition cursor-pointer"
        ...
        :title="w.title"
      >
        <component :is="w.icon" :size="12" class="text-gold-400 shrink-0" />
        <span class="hidden xs:inline sm:inline">{{ w.label }}</span>
      </button>
    </div>
  </div>
</template>
